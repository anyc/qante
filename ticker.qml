...
/*
 * ticker shows scrolling text from a RSS or Atom newsfeed
 */
...
Widget {
	...
	property string defaultImage: "" // default image if RSS entry has no image
	property var feedType: ["atom"]
	property var urls: ["http://rpibuntu.kicherer.org/www/feeds/news.atom.xml"]
	property var namespaces: ["http://www.w3.org/2005/Atom"]
	property bool verboseHeadline: false  // show title or description
	property int scrollSpeed: 150         // ms per letter
						...
						onRunningChanged: {
							...
							if (this.running == false && plate.transform[0].angle == 0) {
								...
								if (tickerText.contentWidth > tickerTextCanvas.width) {
									...
									tickerTextBox.x = (tickerTextCanvas.width - tickerText.contentWidth) - tickerTextCanvas.width/10
								} else {
									if (root.autoLoop) {
										if (root.scrollTime == 0) {
											// adjust scroll duration according to text length
